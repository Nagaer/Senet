...
while choice != 1:
    ...
    qqq = not qqq
if qqq:
    NowPlayer = BluePlayer
    EnemyPlayer = RedPlayer
else:
    NowPlayer = RedPlayer
    EnemyPlayer = BluePlayer
    ...
    for e in pg.event.get(): #Обработчик событий
        if e.type == pg.QUIT: #Выход
            ...
            if e.key == pg.K_SPACE: #Бросок палочек
                random.seed(os.urandom(17))
                ColorDice = (random.randint(0, 1), random.randint(0, 1), random.randint(0, 1), random.randint(0, 1))
        elif e.type == pg.MOUSEBUTTONDOWN: #Обработчик отрисовки мышки
            if InList(e.pos[0], e.pos[1]) != None:
                ...
                if e.button == 1:
                    ...
                    if kol<0:
                        kol = 2
                Play_Field[k][2] = COLOR[kol]
    
    #AI
    k = 1
    while ToPlay and k:
        k -= 1
        q = NowPlayer.move(EnemyPlayer, choice) #Получаем, есть ли право на доп. ход
        if q:
            k += 1
            q = False
        if len(BluePlayer.chips) == 0:
            ...
            NowPlayer, EnemyPlayer = EnemyPlayer, NowPlayer
            choice = EgDice()
